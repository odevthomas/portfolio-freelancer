...
const CompanyCard = ({ image, name, style }) => {
  return (
    <div className="flex flex-col items-center justify-center p-6 hover:scale-105 transition-all duration-300 transform hover:shadow-xl">
      ...
      <h3 className="text-lg text-gray-800 font-semibold text-center">{name}</h3>
      <p className="text-sm text-gray-500 text-center">{style}</p>
    </div>
  );
};
...
const TrustedCompaniesCarousel = () => {
  ...
  return (
    <section id="empresas-confiaram" className="w-full py-16 bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.h2
          className="text-3xl sm:text-4xl font-semibold text-center text-gray-800 mb-12"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
